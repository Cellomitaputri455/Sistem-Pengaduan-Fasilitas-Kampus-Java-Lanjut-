...
export default function AdminMahasiswa() {
  ...
  const handleNonaktifkan = async (id) => {
    ...
    try {
      await api.patch(`/api/mahasiswa/${id}/nonaktifkan`)
      fetchMahasiswa()
    } catch (err) {
      alert(err.response?.data?.message || 'Gagal nonaktifkan mahasiswa')
    }
  }
  ...
  const handleAktifkan = async (id) => {
    ...
    try {
      await api.patch(`/api/mahasiswa/${id}/aktifkan`)
      fetchMahasiswa()
    } catch (err) {
      alert(err.response?.data?.message || 'Gagal mengaktifkan mahasiswa')
    }
  }
      ...
      <div className="px-8 py-8">
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="px-5 py-10 text-center text-sm text-gray-400">Belum ada mahasiswa.</div>
          ) : (
                ...
                {mahasiswa.map(m => (
                  <tr key={m.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-5 py-3">
                      ...
                      <p className="text-xs text-gray-400">{m.noHp}</p>
                    </td>
                    <td className="px-5 py-3">
                      <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${m.isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                    ...
                    <td className="px-5 py-3">
                      {m.isActive ? (
                        <button
                          onClick={() => handleNonaktifkan(m.id)}
