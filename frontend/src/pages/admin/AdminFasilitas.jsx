...
export default function AdminFasilitas() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (modal === 'tambah') {
        await api.post('/api/fasilitas', form)
      } else {
        await api.put(`/api/fasilitas/${selected.id}`, form)
      }
      setModal(null)
      fetchFasilitas()
    } catch (err) {
      setFormError(err.response?.data?.message || 'Gagal menyimpan fasilitas')
    } finally {
  ...
  const handleNonaktifkan = async (id) => {
    ...
    try {
      ...
      alert(err.response?.data?.message || 'Gagal nonaktifkan fasilitas')
    }
  }
    ...
    <AdminLayout>
      <div className="px-8 py-8">
        <div className="mb-6 flex justify-between items-center">
          <div>
            <h2 className="text-xl font-bold text-gray-800">Kelola Fasilitas</h2>
            <p className="text-sm text-gray-500 mt-1">Tambah dan kelola data fasilitas kampus</p>
          </div>
          <button
            onClick={openTambah}
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="px-5 py-10 text-center text-sm text-gray-400">Tidak ada fasilitas.</div>
          ) : (
                ...
                {fasilitas.map(f => (
                  <tr key={f.id} className="hover:bg-gray-50">
                    <td className="px-5 py-3">
                      <p className="font-medium text-gray-800">{f.nama}</p>
                      <p className="text-xs text-gray-400">{f.deskripsi}</p>
                    </td>
                    <td className="px-5 py-3 text-gray-600">{f.kategori}</td>
                    <td className="px-5 py-3 text-gray-600">{f.lokasi}</td>
                    <td className="px-5 py-3">
                      <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${f.isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-md mx-4">
            <h3 className="text-base font-semibold text-gray-800 mb-4">
              {modal === 'tambah' ? 'Tambah Fasilitas' : 'Edit Fasilitas'}
            </h3>
            ...
            <div className="space-y-4">
              {[
                { label: 'Nama', key: 'nama', placeholder: 'Contoh: Lab Komputer A' },
                { label: 'Kategori', key: 'kategori', placeholder: 'Contoh: Laboratorium' },
                { label: 'Lokasi', key: 'lokasi', placeholder: 'Contoh: Gedung B Lt.2' },
              ].map(f => (
                <div key={f.key}>
