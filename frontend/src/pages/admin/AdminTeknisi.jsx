...
export default function AdminTeknisi() {
  ...
  const handleNonaktifkan = async (id) => {
    ...
    try {
      await api.patch(`/api/teknisi/${id}/nonaktifkan`)
      fetchTeknisi()
    } catch (err) {
      alert(err.response?.data?.message || 'Gagal nonaktifkan teknisi')
    }
  }
      ...
      <div className="px-8 py-8">
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="px-5 py-10 text-center text-sm text-gray-400">Belum ada teknisi.</div>
          ) : (
                ...
                {teknisi.map(t => (
                  <tr key={t.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-5 py-3">
                      ...
                      <p className="text-xs text-gray-400">{t.noHp}</p>
                    </td>
                    <td className="px-5 py-3">
                      <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${t.isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto">
            <h3 className="text-base font-semibold text-gray-800 mb-4">
              {modal === 'tambah' ? 'Tambah Teknisi' : 'Edit Teknisi'}
            </h3>
