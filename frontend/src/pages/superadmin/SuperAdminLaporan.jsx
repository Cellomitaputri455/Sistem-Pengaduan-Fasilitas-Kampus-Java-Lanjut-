...
export default function SuperAdminLaporan() {
  ...
  const fetchLaporan = () => {
    ...
    api.get(url).then(res => setLaporan(res.data.data || [])).catch(() => setLaporan([])).finally(() => setLoading(false))
  }
  ...
  const exportPDF = () => {
    ...
    autoTable(doc, {
      ...
      body: laporan.map((p, i) => [i + 1, p.nomorPengaduan, p.judul, p.mahasiswa?.namaLengkap || '-', p.prioritas, p.status, p.teknisi?.namaLengkap || '-', new Date(p.createdAt).toLocaleDateString('id-ID')]),
      styles: { fontSize: 8 },
    ...
    <SuperAdminLayout>
      <div className="px-8 py-8">
        <div className="mb-6 flex justify-between items-center">
          <div><h2 className="text-xl font-bold text-gray-800">Laporan Pengaduan</h2><p className="text-sm text-gray-500 mt-1">Filter dan export data laporan</p></div>
          <button onClick={exportPDF} disabled={laporan.length === 0} className="bg-green-600 hover:bg-green-700 text-white text-sm font-medium px-5 py-2.5 rounded-lg transition disabled:opacity-50">↓ Export PDF</button>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          : laporan.length === 0 ? <div className="px-5 py-10 text-center text-sm text-gray-400">Tidak ada data laporan.</div>
          : (
              ...
              <tbody className="divide-y divide-gray-100">
                {laporan.map((p, i) => (
                  <tr key={p.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-5 py-3 text-gray-500 text-xs">{new Date(p.createdAt).toLocaleDateString('id-ID')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
